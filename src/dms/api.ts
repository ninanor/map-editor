...
async function queryPMTiles(input: string) {
  return axios
    .get<DataTableQuery>(`${window.DMS_API_ENDPOINT}v1/datatables/?search=${input}&is_pmtiles=true&is_accessible=true`)
    .then(r => r.data);
}

export async function queryByType({ queryKey }: QueryOptions): Promise<DatasetQuery | ResourceQuery | DataTableQuery> {
  ...
  if (queryKey && queryKey[0] === 'vector') {
    return await queryPMTiles((queryKey[1] as string) ?? '');
  }
